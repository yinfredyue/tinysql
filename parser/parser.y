...
- Overview: https://pingcap.com/blog-cn/tidb-source-code-reading-5/
  It introduces Lex & Yacc tool to generate SQL compiler. Introduce how to write input files for Lex and Yacc. It
  briefly talks about Syntax Directed Transition (SDT) which can convert the tokens into an AST. It also briefly talks
  about the stack-based parsing procedure.
  TiDB doesn't use tokenizer generated by Lex, but manually wrote one (for performance). This is in parser/lexier.go.
  For parser, TiDB uses goyacc (the Go implementation of Yacc), and the grammar is defined in parser.y.
- How to understand this file?
  This http://dinosaur.compilertools.net/yacc/index.html explains the meaning of %union { ... }% below.
  " By default, the values returned by actions and the lexical analyzer are integers. Yacc can also support values of
  other types, including structures. In addition, Yacc keeps track of the types, and inserts appropriate union member
  names so that the resulting parser will be strictly type checked. The Yacc value stack (see Section 4) is declared
  to be a UNION of the various types of values desired. The user declares the union, and associates union member names
  to each token and nonterminal symbol having a value. When the value is referenced through a $$ or $n construction,
  Yacc will automatically insert the appropriate union name, so that no unwanted conversions will take place. "
  In short, it allows Yacc to return values other than simple integers. It also allows Yacc to do type checking.


- Reference:
  - Lex && Yacc manual: http://dinosaur.compilertools.net/
*/

// This declares the Yacc value stack, and the external variables yylval and yyval, to have type equal to this union.
// Once defined, the union member names must be associated with the various terminal and nonterminal names.
//The construction
//        < name >
//is used to indicate a union member name. If this follows one of the keywords %token, %left, %right, and %nonassoc,
// the union member name is associated with the tokens listed. Thus, saying
//        %left  <optype>  '+'  '-'
//will cause any reference to values returned by these two tokens to be tagged with the union member name optype.
// Another keyword, %type, is used similarly to associate union member names with nonterminals. Thus, one might say
...
// %token: Names representing tokens must be declared; this is most simply done by writing
//        %token   name1  name2 . . .
// in the declarations section. Every name not defined in the declarations section is assumed to represent a
// nonterminal symbol. Every nonterminal symbol must appear on the left side of at least one rule.

// Of all the nonterminal symbols, one, called the start symbol, has particular importance. The parser is designed to
// recognize the start symbol; thus, this symbol represents the largest, most general structure described by the grammar
// rules. By default, the start symbol is taken to be the left hand side of the first grammar rule in the rules section.
// It is possible, and in fact desirable, to declare the start symbol explicitly in the declarations section using the
// %start keyword:
//        %start   symbol


// %left, %right describes the associativity. All tokens on the same line are assumed to have the same precedence level
// and associativity. The lines are listed in order of increasing precedence or binding strength.
// Example:
//        %left  '+'  '-'
//        %left  '*'  '/'
%token	<ident>
...
JoinTable:
	...
	TableRef CrossOpt TableRef %prec tableRefPriority
	{
		// http://dinosaur.compilertools.net/yacc/index.html
                // This makes the precedence of the rule to be the same as tableRefPrority
		$$ = &ast.Join{Left: $1.(ast.ResultSetNode), Right: $3.(ast.ResultSetNode), Tp: ast.CrossJoin}
	}
